...
Promise.resolve(
  (async () => {
    ...
    const page = pages.find((page) =>
      page.url().includes('wxde8ac0a21135c07d'),
    );
    ...
    // 👀 run YAML with agent
    // - ai: 我需要订购外卖送到家，外卖的商品是任意奶茶饮品，高价优先，商品定制默认。如果商品价格不够起送费，订购两杯。不要参与抢券或者红包。
    const { result } = await agent.runYaml(`
tasks:
  - name: buy-drink
    flow:
      - ai: 订购任意奶茶饮品送到家，商品不定制使用默认配置，如果商品价格不够起送费，订购两杯，不要参与抢券或者抢红包。
      - sleep: 3000
`);
